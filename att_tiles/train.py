...
from models import TileEncoder, DecoderWithAttention
...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        CaptionDataset(data_folder, 'train', train_transform, tile_folder, cluster_folder), 
        batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True)

    val_loader = torch.utils.data.DataLoader(
        CaptionDataset(data_folder, 'val', val_transform, tile_folder, cluster_folder), 
        batch_size=batch_size, shuffle=True, num_workers=workers, pin_memory=True)
    ...
    for epoch in range(start_epoch, epochs):
        ...
        save_checkpoint(data_name, epoch, epochs_since_improvement, 
                tile_encoder, decoder, 
                tile_encoder_optimizer, 
                decoder_optimizer, recent_bleu4, is_best)


def train(train_loader, tile_encoder, decoder, criterion, tile_encoder_optimizer, decoder_optimizer, epoch):
    decoder.train()  
    tile_encoder.train()
    ...
    for i, (imgs, caps, caplens, tiles, proportions) in enumerate(train_loader):
        ...
        scores, caps_sorted, decode_lengths, alphas, sort_ind = decoder(tiles, caps, caplens)
...
def validate(val_loader, tile_encoder, decoder, criterion):
    decoder.eval()  
